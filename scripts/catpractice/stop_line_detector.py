...
class Stop_line_detactor(Scan_image):
    ...
    def image_callback(self, msg):
        ...
        ret, thr = cv2.threshold(self.mask, 127, 255, 0)
        ...
        if 11000.0 < area and len(contours) == 1:   # need to find area's max range
            msg = "stop"
        ...
        for cnt in contours:
            ...
            cv2.drawContours(self.image, [approx1], 0, (0, 255, 0), 3)
            x, y, w, h = cv2.boundingRect(cnt)
            a = cv2.contourArea(cnt)
            if a > 100:
                cv2.putText(self.image, str(a), (x, y), font, fontScale, (255, 0, 255), 2)
        '''
